...
assert(trainingGpuRenderer.includes("temporalSnapshot.volume.fennec") && trainingGpuFennecVolumeUtils.includes("state.surface") && trainingGpuFennecVolumeUtils.includes("state.contour") && trainingRadarSnapshots.includes("getTrainingRadarVolumeScanState"), "The Fennec scan must join the canonical absolute MasterClock volume snapshot and reproduce active, hold, fade and hidden states.");
...
assert(trainingScene.includes("assets.fennecBase") && !css.includes("--training-fennec-gpu-base-opacity") && !trainingGpuFennecVolumeUtils.includes("--training-fennec-gpu-base-opacity") && !trainingGpuFennecVolumeUtils.includes("setDomBaseOpacity"), "The temporary DOM opacity bridge must be fully removed once the base opacity feeds the shared WebGL render directly.");
...
assert(trainingGpuObjectRegistry.includes('fennecBaseGroup = "fennec:base-scene"') && trainingGpuFennecVolumeUtils.includes("this.baseQuad = getQuad(assetSet.assets.base)") && trainingGpuFennecVolumeUtils.includes("state.baseOpacity"), "The GPU Fennec base must use its scene placement from the manifest and the shared absolute tactical opacity.");
const fennecFrameRenderStart = trainingGpuRenderer.indexOf("private renderFrame(frameState:");
const fennecFrameRender = trainingGpuRenderer.slice(fennecFrameRenderStart, trainingGpuRenderer.indexOf("\n  private renderRadarFrame", fennecFrameRenderStart));
const fennecCanAnimate = trainingGpuRenderer.slice(trainingGpuRenderer.indexOf("private canAnimate()"), trainingGpuRenderer.indexOf("\n  private syncAnimationLoop()"));
assert(fennecFrameRender.indexOf("renderBase(fennecEffectsState)") < fennecFrameRender.indexOf("fennecVolumeSubsystem.render(") && fennecFrameRender.indexOf("fennecVolumeSubsystem.render(") < fennecFrameRender.indexOf("renderEffects(") && fennecEffectsRender.indexOf("effectsResources.impactTexture") < fennecEffectsRender.indexOf("effectsResources.rearTexture") && fennecEffectsRender.indexOf("effectsResources.rearTexture") < fennecEffectsRender.indexOf("effectsResources.headlightTexture"), "The single Fennec canvas must preserve base, volume surface, volume contour, tactical impact, rear accent and headlight glow draw order.");
assert(trainingGpuFennecVolumeUtils.includes("gl.ONE_MINUS_SRC_ALPHA") && trainingGpuRenderer.includes("renderBase(fennecEffectsState, true)") && trainingGpuRenderer.includes("renderStaticObjectFrame") && !fennecCanAnimate.includes("isBaseReady"), "The Fennec base must render with normal premultiplied blending in first and static frames without starting a base-only permanent RAF.");
assert(trainingScene.includes("gpuFennecBaseReady") && trainingScene.includes('data-gpu-fennec-base-ready=') && css.includes('.training-scene[data-gpu-fennec-base-ready="true"]') && css.includes('.training-scene[data-gpu-fennec-base-ready="false"]') && css.includes(".training-fennec-base-frame") && css.includes("mix-blend-mode: screen"), "The preserved DOM Fennec base fallback must hide only after an independent successful GPU base draw while GPU effects keep their screen composite over that fallback.");
assert(trainingGpuFennecVolumeUtils.includes('recordContextLost("fennec-base")') && trainingGpuFennecVolumeUtils.includes('recordContextRestored("fennec-base")') && trainingGpuFennecVolumeUtils.includes("textures: this.baseTexture ? 1 : 0") && trainingGpuDebugTypes.includes('| "fennec-base"') && trainingGpuDebugCollector.includes('"fennec-base": createSubsystemState()') && trainingGpuDebugPanel.includes('"fennec-base"'), "debugRenderer=1 must expose Fennec base readiness, CPU, one texture, memory, static renders, errors and shared-context recovery.");
assert((trainingScene.match(/training-gpu-fennec-canvas/g) ?? []).length === 1 && (trainingGpuFennecVolumeUtils.match(/\.getContext\(/g) ?? []).length === 1 && trainingGpuFennecVolumeUtils.includes("assetSet.assets.base") && trainingGpuObjectAssetCatalog.includes('"fennec"'), "GPU mode must render all five prepared objects while retaining exactly one Fennec canvas and one Fennec context.");
...
assert(trainingGpuRenderer.includes("const temporalSnapshot = getTrainingRadarTemporalSnapshot(frameState)") && trainingGpuRenderer.includes("this.options.applyDomSnapshot?.(temporalSnapshot)") && trainingGpuRenderer.includes("this.renderFrame(temporalSnapshot)"), "The GPU RAF must calculate one shared frame snapshot and hand that same value to GPU and DOM.");
assert(trainingGpuRenderer.includes("this.options.applyDomSnapshot !== null ||") && trainingGpuRenderer.includes("this.options.applyDomSnapshot !== null ||\n        this.radarReady"), "GPU mode must keep its single renderer RAF alive for synchronized DOM fallbacks even when every GPU subsystem is unavailable.");
