...
assert(trainingParticleField.includes("TRAINING_RADAR_SWEEP") && trainingParticleField.includes("getTrainingRadarDelayForProgress"), "Particle delays must derive from the same central sweep geometry and nonlinear timeline as the radar.");
assert(
  trainingParticleField.includes("getParticleScanDelayMs") &&
    trainingParticleField.includes("getRadarCoreOffsetX") &&
    trainingParticleField.includes("getTrainingRadarDelayForProgress(progress, direction)"),
...
assert(trainingParticleField.includes('direction === "ltr"') && trainingParticleField.includes("logicalX - TRAINING_RADAR_SWEEP.startX + coreOffsetX"), "Particle delay must reverse with the radar direction while preserving the slanted core contact.");
...
assert(trainingRadarSequence.includes("volumeHitDelayMs - TRAINING_VOLUME_SCAN_TIMING.leadMs") && trainingRadarSequence.includes("Math.max(") && trainingRadarSequence.includes("volumeStartDelayMs + volumeActiveDurationMs"), "Systematic volume scans must lead the nonlinear theoretical hit while keeping their lifecycle synchronized.");
...
for (const timing of ["TRAINING_RADAR_BASE_TRAVEL_DURATION_MS = 2500", "TRAINING_RADAR_PASS_BUFFER_MS = 200", "contactDurationMs: 180", "wireframeDelayMs: 820", "fadeDelayMs: 1500", "targetLifetimeMs: 2300", "fadeDurationMs: 800", "activeDurationMs: 380", "ballActiveDurationMs: 540", "fennecActiveDurationMs: 720", "contourDelayMs: 60", "fadeDurationMs: 210", "leadMs: 120", "totalDurationMs: 610"]) {
  assert(trainingRadarTargets.includes(timing), `Missing centralized radar timing: ${timing}`);
}
assert(trainingRadarTargets.includes('id: "ball"') && trainingRadarTargets.includes("centerProgress: TRAINING_BALL_SCAN_PROGRESS") && trainingRadarTargets.includes("width: 0.1") && trainingRadarTargets.includes("extraDurationMs: 220"), "The central timeline must add a moderate slowdown around the ball.");
assert(trainingRadarTargets.includes('id: "fennec"') && trainingRadarTargets.includes("centerProgress: TRAINING_FENNEC_SCAN_PROGRESS") && trainingRadarTargets.includes("width: 0.16") && trainingRadarTargets.includes("extraDurationMs: 380"), "The central timeline must add a stronger slowdown around the Fennec.");
assert(trainingRadarTargets.includes("getTrainingRadarTravelDelayForProgress") && trainingRadarTargets.includes("getTrainingRadarDelayForProgress") && trainingRadarTargets.includes("smoothTrainingRadarStep") && !trainingRadarTargets.includes("TRAINING_RADAR_TIMING.travelDurationMs * hitProgress"), "Radar hit delays must use the shared nonlinear timeline instead of the old linear formula.");
assert(trainingRadarTargets.includes("TRAINING_RADAR_PASS_BUFFER_MS") && trainingRadarTargets.includes("TRAINING_RADAR_TRAVEL_DURATION_MS +\n    TRAINING_RADAR_PASS_BUFFER_MS"), "Pass duration must remain longer than entry plus travel by the centralized end buffer.");
assert(trainingRadarTargets.includes('ltr: getTrainingRadarTravelEasing("ltr")') && trainingRadarTargets.includes('rtl: getTrainingRadarTravelEasing("rtl")') && trainingRadarTargets.includes('return `linear(${points.join(", ")})`'), "Both visual radar directions must derive their CSS easing from the same nonlinear timeline.");
assert(trainingRadarOverlay.includes('"--radar-travel-easing": TRAINING_RADAR_TRAVEL_EASING[direction]') && css.includes("var(--radar-travel-easing)") && css.includes("@keyframes training-radar-visibility"), "The visual radar movement must consume the central timeline while opacity stays on an independent animation.");
...
assert(/@keyframes training-ball-volume-surface-rtl\s*\{[\s\S]*?mask-position:\s*12% 50%;[\s\S]*?mask-position:\s*88% 50%;/s.test(css), "The mirrored RTL ball mask must enter the sphere immediately from the opposite edge.");
...
const fennecRtlDirectionCss = css.slice(css.indexOf('.training-radar-fennec-target[data-radar-direction="rtl"]'), css.indexOf(".training-radar-fennec-surface-mask {"));
...
assert(/\.training-radar-fennec-surface-mask\s*\{[\s\S]*?--training-fennec-surface-mask-angle:\s*90deg;[\s\S]*?position:\s*absolute;[\s\S]*?inset:\s*0;[\s\S]*?width:\s*100%;[\s\S]*?height:\s*100%;[\s\S]*?opacity:\s*0\.48;[\s\S]*?linear-gradient\(\s*var\(--training-fennec-surface-mask-angle\)[\s\S]*?black 0 50%,[\s\S]*?transparent 53%[\s\S]*?mask-size:\s*200% 100%;/s.test(css), "The cumulative radar mask must remain full-canvas with one fixed 90-degree orientation.");
assert(!fennecSurfaceMaskCss.includes("--training-fennec-volume-mask-angle") && !fennecRtlDirectionCss.includes("--training-fennec-surface-mask-angle"), "RTL may rotate contour and im-light masks, but must never invert the fixed cumulative surface mask.");
...
assert(trainingScene.includes('className="training-radar-fennec-impact-frame"') && /\.training-radar-fennec-impact-frame\s*\{[\s\S]*?left:\s*15\.8085%;[\s\S]*?top:\s*6\.1722%;[\s\S]*?width:\s*80\.4954%;[\s\S]*?height:\s*88\.9024%;[\s\S]*?z-index:\s*2;[\s\S]*?pointer-events:\s*none;/s.test(css), "The im-light must share the exact calibrated frame of the surface scan while staying one layer below it.");
...
assert(/@keyframes training-fennec-volume-surface-ltr\s*\{[\s\S]*?mask-position:\s*var\(--training-fennec-mask-start-position\) 50%;[\s\S]*?mask-position:\s*var\(--training-fennec-mask-end-position\) 50%;/s.test(css) && /@keyframes training-fennec-volume-surface-rtl\s*\{[\s\S]*?mask-position:\s*var\(--training-fennec-mask-end-position\) 50%;[\s\S]*?mask-position:\s*var\(--training-fennec-mask-start-position\) 50%;/s.test(css) && !fennecSurfaceKeyframes.includes("opacity:"), "LTR and RTL must animate only the full-canvas mask position at constant opacity.");
assert(trainingRadarTargets.includes("getTrainingRadarRangeTiming") && trainingRadarTargets.includes("startProgress: 0.613") && trainingRadarTargets.includes("endProgress: 0.924") && trainingRadarSequence.includes("fennecRangeTiming?.startDelayMs") && trainingRadarSequence.includes("fennecRangeTiming?.durationMs"), "The Fennec volume lifecycle must use the exact nonlinear radar timing across its measured width.");
assert(trainingScene.includes("TRAINING_RADAR_TIMING.passDurationMs") && trainingScene.includes("trainingFennecLtrRangeTiming.startDelayMs") && trainingScene.includes("trainingFennecLtrRangeTiming.durationMs") && trainingScene.includes("trainingFennecRtlRangeTiming.startDelayMs") && trainingScene.includes('"--training-fennec-persisted-duration"') && !trainingScene.includes("830ms"), "The persisted crossfade duration must be derived from the central LTR/RTL radar timeline instead of being hardcoded.");
assert(trainingRadarSequence.includes("TrainingFennecSurfaceMode") && trainingRadarSequence.includes("surfaceMode ?? current.fennecSurfaceMode") && /setVolumeScan\([\s\S]*?volumeTarget\.id,[\s\S]*?"active",[\s\S]*?passDirection,[\s\S]*?volumeTarget\.id === "fennec"[\s\S]*?"reveal"[\s\S]*?"erase"/s.test(trainingRadarSequence), "Direction, active volume phase and Fennec surface mode must commit in one atomic state update.");
assert(trainingScene.includes("--training-volume-scan-easing") && trainingScene.includes("--training-fennec-mask-start-position") && trainingScene.includes("data-surface-scan-mode={fennecSurfaceMode}") && /data-surface-scan-mode="persisted"[\s\S]*?training-radar-fennec-surface-mask[\s\S]*?mask-position:\s*var\(--training-fennec-mask-end-position\) 50%;/s.test(fennecScanCss), "Persisted mode must keep the same installed cumulative mask fully open.");
assert(/data-surface-scan-mode="reveal"[\s\S]*?training-radar-fennec-surface-mask[\s\S]*?training-fennec-volume-surface-ltr/s.test(fennecScanCss) && /data-surface-scan-mode="erase"[\s\S]*?training-radar-fennec-surface-mask[\s\S]*?training-fennec-volume-surface-rtl/s.test(fennecScanCss) && !/data-volume-scan-phase="active"[\s\S]*?training-radar-fennec-surface-mask/s.test(fennecScanCss), "Surface reveal and erase must animate only the dedicated mask through mutually exclusive modes.");
assert(trainingScene.includes('className="training-fennec-base-frame"') && trainingScene.includes('className="training-fennec-base"') && trainingScene.includes("data-surface-scan-mode={fennecSurfaceMode}"), "The Fennec base must expose the same reveal mode without changing its calibrated canvas.");
...
assert(!css.includes('.training-fennec-base-frame[data-surface-scan-mode="reveal"]') && !fennecScanCss.includes('.training-radar-fennec-target[data-surface-scan-mode="reveal"]\n  .training-radar-fennec-impact'), "Base and im-light opacity must remain untouched throughout reveal.");
assert(/\.training-fennec-base-frame\[data-surface-scan-mode="persisted"\]\s*\{[\s\S]*?training-fennec-base-persisted-crossfade[\s\S]*?var\(--training-fennec-persisted-duration\) cubic-bezier\(0\.45, 0, 0\.55, 1\) both;/s.test(css) && /data-surface-scan-mode="persisted"\][\s\S]*?\.training-radar-fennec-impact\s*\{[\s\S]*?training-fennec-impact-persisted-crossfade[\s\S]*?var\(--training-fennec-persisted-duration\) cubic-bezier\(0\.45, 0, 0\.55, 1\) both;[\s\S]*?mask-image:\s*none;/s.test(fennecScanCss), "Persisted mode must start both complementary crossfades together with one computed duration and easing curve.");
assert(/@keyframes training-fennec-base-persisted-crossfade\s*\{\s*0%\s*\{\s*opacity:\s*1;\s*\}\s*50%\s*\{\s*opacity:\s*0;\s*\}\s*100%\s*\{\s*opacity:\s*1;\s*\}\s*\}/s.test(css) && /@keyframes training-fennec-impact-persisted-crossfade\s*\{\s*0%\s*\{\s*opacity:\s*0;\s*\}\s*50%\s*\{\s*opacity:\s*1;\s*\}\s*100%\s*\{\s*opacity:\s*0;\s*\}\s*\}/s.test(css), "Base and im-light persisted keyframes must be strict opacity inverses.");
assert(/\.training-fennec-base-frame\[data-surface-scan-mode="erase"\]\s*\{\s*opacity:\s*1;\s*animation:\s*none;/s.test(css) && /data-surface-scan-mode="erase"[\s\S]*?\.training-radar-fennec-impact\s*\{\s*opacity:\s*0;\s*animation:\s*none;/s.test(fennecScanCss), "Erase must begin with the base restored and im-light hidden, without replaying either animation.");
assert(!/\.training-radar-fennec-target\[data-volume-scan-phase="(?:active|fade)"\](?:\[data-radar-direction="(?:ltr|rtl)"\])?\s*\.training-radar-fennec-impact\s*\{/s.test(fennecScanCss), "The generic volume phase must no longer control or block the persisted im-light.");
...
assert(!/data-volume-scan-phase="fade"\][\s\S]*?:is\(\.training-radar-fennec-surface/s.test(fennecScanCss), "The generic fade phase must not erase the memorized Fennec surface globally.");
