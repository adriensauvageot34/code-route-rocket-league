...
assert(/\.training-radar-fennec-surface\s*\{[\s\S]*?clip-path:\s*none;[\s\S]*?brightness\(1\.18\) contrast\(1\.34\) saturate\(1\.2\);[\s\S]*?black 0 50%,[\s\S]*?transparent 53%[\s\S]*?mask-size:\s*200% 100%;/s.test(css), "The Fennec surface overlay must use only its native alpha and the directional reveal mask at the shared canvas scale.");
...
assert(/@keyframes training-fennec-volume-surface-ltr\s*\{[\s\S]*?mask-position:\s*var\(--training-fennec-mask-start-position\) 50%;[\s\S]*?opacity:\s*0\.48;[\s\S]*?mask-position:\s*var\(--training-fennec-mask-end-position\) 50%;/s.test(css) && /@keyframes training-fennec-volume-surface-rtl\s*\{[\s\S]*?mask-position:\s*var\(--training-fennec-mask-end-position\) 50%;[\s\S]*?opacity:\s*0;[\s\S]*?mask-position:\s*var\(--training-fennec-mask-start-position\) 50%;/s.test(css), "LTR must write and RTL must erase across the complete measured Fennec width using only the overlay alpha.");
...
assert(trainingScene.includes("--training-volume-scan-easing") && trainingScene.includes("--training-fennec-mask-start-position") && trainingScene.includes("data-surface-scan-mode={fennecSurfaceMode}") && /data-surface-scan-mode="persisted"[\s\S]*?opacity:\s*0\.48;[\s\S]*?mask-image:\s*none;/s.test(fennecScanCss), "The Fennec mask must share the radar easing and persist through its own native alpha after LTR.");
