...
assert(trainingRadarSequence.includes('volumeTarget.type === "ball"') && trainingRadarSequence.includes('volumeTarget.type === "fennec"') && trainingGroundedActor.includes("TRAINING_VOLUME_SCAN_TIMING.ballActiveDurationMs") && trainingScene.includes("TRAINING_VOLUME_SCAN_TIMING.fennecActiveDurationMs"), "Only the ball and Fennec volume reveals must remain active longer during their slow analysis zones.");
...
assert(/\.training-radar-fennec-surface\s*\{[\s\S]*?clip-path:\s*none;[\s\S]*?brightness\(1\.18\) contrast\(1\.34\) saturate\(1\.2\);[\s\S]*?black 0 50%,[\s\S]*?transparent 53%[\s\S]*?mask-size:\s*200% 100%;/s.test(css), "The Fennec surface overlay must stay uncropped, crisp and readable across the full native canvas.");
...
assert(trainingRadarSequence.includes("fennecSurfacePersistent: boolean") && /schedule\(\(\) => \{[\s\S]*?setFennecSurfacePersistent\(passDirection === "ltr"\);[\s\S]*?setVolumeScan\(volumeTarget\.id, "fade"\);[\s\S]*?volumeStartDelayMs \+ volumeActiveDurationMs\);/.test(trainingRadarSequence), "The memorized Fennec surface state must switch only after the shared radar progression completes.");
assert(trainingScene.includes("--training-volume-scan-easing") && trainingScene.includes("--training-fennec-mask-start-position") && trainingScene.includes('data-surface-scan-persistent={fennecSurfacePersistent ? "true" : "false"}') && /data-surface-scan-persistent="true"[\s\S]*?:not\([\s\S]*?data-volume-scan-phase="active"[\s\S]*?opacity:\s*0\.48;[\s\S]*?mask-image:\s*none;/s.test(fennecScanCss), "The Fennec mask must share the radar easing and persist after a completed LTR scan.");
