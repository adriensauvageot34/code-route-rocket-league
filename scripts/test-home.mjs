...
assert(/\.training-fennec-base-frame\[data-surface-scan-mode="reveal"\]\s*\{[\s\S]*?training-fennec-base-scan-emphasis var\(--training-volume-scan-duration\)[\s\S]*?var\(--training-volume-scan-easing\) both;/s.test(css) && /@keyframes training-fennec-base-scan-emphasis\s*\{[\s\S]*?15%\s*\{\s*opacity:\s*0\.42;[\s\S]*?65%\s*\{\s*opacity:\s*0\.42;[\s\S]*?88%\s*\{\s*opacity:\s*0\.82;[\s\S]*?100%\s*\{\s*opacity:\s*1;/s.test(css), "LTR reveal must visibly calm the base frame and restore it before persistence.");
