...
assert(/\.training-radar-ball-volume-surface,[\s\S]*?\.training-radar-ball-volume-contour\s*\{[\s\S]*?inset:\s*0;[\s\S]*?width:\s*100% !important;[\s\S]*?height:\s*100% !important;[\s\S]*?object-fit:\s*cover;/s.test(css), "Ball scan assets must share the full-canvas base-ball geometry.");
