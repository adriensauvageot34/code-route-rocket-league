...
assert(/@keyframes training-fennec-volume-surface-ltr\s*\{[\s\S]*?mask-position:\s*var\(--training-fennec-mask-start-position\) 50%;[\s\S]*?opacity:\s*0\.48;[\s\S]*?mask-position:\s*var\(--training-fennec-mask-end-position\) 50%;/s.test(css) && /@keyframes training-fennec-volume-surface-rtl\s*\{[\s\S]*?mask-position:\s*var\(--training-fennec-mask-end-position\) 50%;[\s\S]*?opacity:\s*0;[\s\S]*?mask-position:\s*var\(--training-fennec-mask-start-position\) 50%;/s.test(css), "LTR must write and RTL must erase across the complete measured Fennec width.");
