...
assert(/\.training-radar-fennec-surface\s*\{[\s\S]*?brightness\(1\.18\) contrast\(1\.14\) saturate\(1\.2\);[\s\S]*?transparent 44%,[\s\S]*?black 48% 54%,[\s\S]*?transparent 57%/s.test(css), "The Fennec surface overlay must remain the sharp, high-contrast primary material reveal.");
assert(/\.training-radar-fennec-contour\s*\{[\s\S]*?--training-fennec-detail-peak:\s*0\.22;[\s\S]*?--training-fennec-detail-tail:\s*0\.06;[\s\S]*?transparent 44%,[\s\S]*?black 49% 52%,[\s\S]*?transparent 56%/s.test(css), "The Fennec contour must remain a lighter reinforcement behind the surface reveal.");
assert(/\.training-radar-fennec-surface,[\s\S]*?\.training-radar-fennec-impact\s*\{[\s\S]*?position:\s*absolute;[\s\S]*?inset:\s*0;[\s\S]*?width:\s*100% !important;[\s\S]*?height:\s*100% !important;[\s\S]*?object-fit:\s*cover;[\s\S]*?object-position:\s*center;[\s\S]*?transform-origin:\s*76\.82% 64\.61%;/s.test(css), "Fennec scan overlays must share the base full-canvas rendering frame.");
assert(fennecScanCss.includes("translate(0.69%, -1.54%) scale(0.807)") && fennecScanCss.includes("translate(6.56%, 7.55%) scale(0.57)"), "Surface, contour and im-light overlays must retain their measured calibration against the base Fennec.");
...
assert(/\.training-radar-fennec-impact\s*\{[\s\S]*?--training-fennec-detail-peak:\s*0\.055;[\s\S]*?--training-fennec-detail-tail:\s*0;[\s\S]*?clip-path:\s*polygon\(37% 31%,[\s\S]*?filter:\s*brightness\(0\.92\) saturate\(1\);/s.test(css), "The im-light must be tightly clipped to the useful Fennec envelope and remain very faint.");
