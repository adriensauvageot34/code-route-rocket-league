...
assert(/\.training-fennec-base-frame\[data-surface-scan-mode="reveal"\][\s\S]*?training-fennec-base-scan-emphasis var\(--training-volume-scan-duration\)[\s\S]*?var\(--training-volume-scan-easing\) both;/s.test(css) && /@keyframes training-fennec-base-scan-emphasis\s*\{[\s\S]*?opacity:\s*0\.68;[\s\S]*?opacity:\s*0\.72;[\s\S]*?100%\s*\{\s*opacity:\s*1;/s.test(css), "LTR reveal must temporarily calm the base Fennec and restore it before persistence.");
assert(/\.training-radar-fennec-impact\s*\{[\s\S]*?--training-fennec-detail-peak:\s*0\.3;[\s\S]*?clip-path:\s*inset\(45% 5\.5% 13% 59\.5%\);[\s\S]*?filter:\s*none;/s.test(css), "The im-light must become readable while remaining tightly clipped and free of diffuse glow.");
assert(/data-surface-scan-mode="reveal"\][\s\S]*?\.training-radar-fennec-impact\s*\{[\s\S]*?training-fennec-volume-impact-ltr var\(--training-volume-scan-duration\)[\s\S]*?var\(--training-volume-scan-easing\) both;/s.test(fennecScanCss), "The im-light highlight must follow the full LTR surface reveal instead of starting late.");
assert(/data-volume-scan-phase="active"\]\[data-radar-direction="rtl"\][\s\S]*?\.training-radar-fennec-impact\s*\{[\s\S]*?opacity:\s*0;[\s\S]*?animation:\s*none;/s.test(fennecScanCss) && /data-volume-scan-phase="fade"\][\s\S]*?\.training-radar-fennec-impact\s*\{[\s\S]*?opacity:\s*0;[\s\S]*?animation:\s*none;/s.test(fennecScanCss), "RTL erase and the later fade phase must not replay or persist the temporary im-light highlight.");
