...
assert(/\.training-radar-fennec-surface\s*\{[\s\S]*?brightness\(1\.12\) contrast\(1\.26\) saturate\(1\.18\);[\s\S]*?black 0 50%,[\s\S]*?transparent 53%[\s\S]*?mask-size:\s*200% 100%;/s.test(css), "The Fennec surface overlay must use a cumulative, sharp-edged material mask rather than a transient scan band.");
...
assert(/\.training-radar-fennec-surface,[\s\S]*?\.training-radar-fennec-impact\s*\{[\s\S]*?position:\s*absolute;[\s\S]*?inset:\s*0;[\s\S]*?width:\s*100% !important;[\s\S]*?height:\s*100% !important;[\s\S]*?object-fit:\s*cover;[\s\S]*?object-position:\s*center;[\s\S]*?clip-path:\s*inset\(47% 7\.6% 15% 61\.3%\);[\s\S]*?transform:\s*none;[\s\S]*?transform-origin:\s*center;/s.test(css), "Fennec scan overlays must use the base full-canvas frame and measured useful alpha bounds.");
...
assert(/@keyframes training-fennec-volume-surface-ltr\s*\{[\s\S]*?mask-position:\s*100% 50%;[\s\S]*?opacity:\s*0\.4;[\s\S]*?mask-position:\s*0% 50%;/s.test(css) && /@keyframes training-fennec-volume-surface-rtl\s*\{[\s\S]*?mask-position:\s*0% 50%;[\s\S]*?opacity:\s*0;[\s\S]*?mask-position:\s*100% 50%;/s.test(css), "LTR must progressively write the full Fennec surface while RTL progressively erases it.");
assert(trainingRadarSequence.includes("fennecSurfacePersistent: boolean") && /schedule\(\(\) => \{[\s\S]*?setFennecSurfacePersistent\(passDirection === "ltr"\);[\s\S]*?setVolumeScan\(volumeTarget\.id, "fade"\);[\s\S]*?volumeStartDelayMs \+ volumeActiveDurationMs\);/.test(trainingRadarSequence), "The memorized Fennec surface state must switch only after the directional scan completes.");
assert(trainingScene.includes('data-surface-scan-persistent={fennecSurfacePersistent ? "true" : "false"}') && /data-surface-scan-persistent="true"[\s\S]*?:not\([\s\S]*?data-volume-scan-phase="active"[\s\S]*?opacity:\s*0\.4;[\s\S]*?mask-image:\s*none;/s.test(fennecScanCss), "A completed LTR scan must remain fully visible until the next active RTL erase.");
