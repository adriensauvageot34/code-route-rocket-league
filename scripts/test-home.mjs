...
assert(trainingGroundedActor.includes('data-object-scan-v1={usesObjectScanV1 ? "true" : "false"}') && trainingGroundedActor.includes('className="training-object-local-scan-line"'), "Car-03 must expose one visible local scan line while other cars keep their tactical container.");
assert(/usesObjectScanV1 \? \([\s\S]*?training-radar-object-surface[\s\S]*?training-radar-object-contour[\s\S]*?\) : null}[\s\S]*?training-radar-car-wireframe[\s\S]*?training-radar-car-glow/s.test(trainingGroundedActor), "Advanced surface and contour scans must stay exclusive to car-03 while every car renders tactical wireframe and glow.");
...
for (const layeredScanMarker of ["training-object-contact", "training-object-local-scan-line-ltr", "training-object-local-scan-line-rtl", "training-object-surface-scan-ltr", "training-object-surface-scan-rtl", "training-object-contour-scan-ltr", "training-object-contour-scan-rtl", "training-object-tactical-wireframe", "training-object-tactical-glow"]) {
  assert(css.includes(layeredScanMarker), `Layered Training object scan CSS missing: ${layeredScanMarker}.`);
}
assert(css.includes("clip-path: inset(0 45% 0 45%)") && css.includes("opacity: 0.66") && css.includes("opacity: 0.46"), "Surface and contour scans must use clearly readable narrow local bands around the moving local line.");
assert(css.includes("width: 3px") && css.includes("650ms ease-out both") && css.includes("left: 104%"), "Car-03 local scan line must visibly traverse the car in either direction for the full surface phase.");
assert(css.includes('.training-radar-car-target[data-radar-active="true"]') && !css.includes('.training-radar-car-target[data-object-scan-v1="true"][data-radar-active="true"]\n  .training-radar-car-wireframe'), "Every adversary car must receive tactical wireframe and glow activation.");
...
assert(css.includes("clip-path: inset(0 69% 0 21%)") && css.includes("clip-path: inset(0 21% 0 69%)"), "The local surface scan must track the visible local line immediately in either radar direction.");
