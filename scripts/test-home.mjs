...
assert(/\.training-radar-fennec-surface\s*\{[\s\S]*?clip-path:\s*none;[\s\S]*?brightness\(1\.18\) contrast\(1\.34\) saturate\(1\.2\);[\s\S]*?url\("\/ui\/training-fennec-base\.png"\),[\s\S]*?black 0 50%,[\s\S]*?transparent 53%[\s\S]*?mask-composite:\s*intersect;[\s\S]*?mask-size:\s*cover, 200% 100%;/s.test(css), "The Fennec surface overlay must be intersected with the exact base-car alpha silhouette at the native canvas scale.");
...
assert(/\.training-radar-fennec-surface,[\s\S]*?\.training-radar-fennec-impact\s*\{[\s\S]*?position:\s*absolute;[\s\S]*?inset:\s*0;[\s\S]*?width:\s*100% !important;[\s\S]*?height:\s*100% !important;[\s\S]*?object-fit:\s*cover;[\s\S]*?object-position:\s*center;[\s\S]*?transform:\s*none;[\s\S]*?transform-origin:\s*center;/s.test(css) && /\.training-radar-fennec-contour\s*\{[\s\S]*?clip-path:\s*none;/s.test(css), "Fennec surface and contour overlays must share the uncropped base canvas without scale or translation.");
assert(!fennecScanCss.includes("scale(") && !fennecScanCss.includes("translate("), "Same-canvas Fennec overlays must not use manual scale or translate calibration.");
assert(/@keyframes training-fennec-volume-surface-ltr\s*\{[\s\S]*?mask-position:\s*center,\s*var\(--training-fennec-mask-start-position\) 50%;[\s\S]*?opacity:\s*0\.48;[\s\S]*?mask-position:\s*center,\s*var\(--training-fennec-mask-end-position\) 50%;/s.test(css) && /@keyframes training-fennec-volume-surface-rtl\s*\{[\s\S]*?mask-position:\s*center,\s*var\(--training-fennec-mask-end-position\) 50%;[\s\S]*?opacity:\s*0;[\s\S]*?mask-position:\s*center,\s*var\(--training-fennec-mask-start-position\) 50%;/s.test(css), "LTR must write and RTL must erase across the complete measured Fennec width while preserving the base silhouette mask.");
...
assert(trainingRadarSequence.includes("TrainingFennecSurfaceMode") && trainingRadarSequence.includes('passDirection === "ltr" ? "reveal" : "erase"') && trainingRadarSequence.includes('passDirection === "ltr" ? "persisted" : "hidden"'), "Fennec reveal and erase must use one exclusive directional surface mode with no competing persistent boolean.");
assert(trainingScene.includes("--training-volume-scan-easing") && trainingScene.includes("--training-fennec-mask-start-position") && trainingScene.includes("data-surface-scan-mode={fennecSurfaceMode}") && /data-surface-scan-mode="persisted"[\s\S]*?opacity:\s*0\.48;[\s\S]*?training-fennec-base\.png/s.test(fennecScanCss), "The Fennec mask must share the radar easing and persist through the base-alpha silhouette after LTR.");
assert(/data-surface-scan-mode="reveal"[\s\S]*?training-fennec-volume-surface-ltr/s.test(fennecScanCss) && /data-surface-scan-mode="erase"[\s\S]*?training-fennec-volume-surface-rtl/s.test(fennecScanCss) && !/data-volume-scan-phase="active"[\s\S]*?training-radar-fennec-surface/s.test(fennecScanCss), "Surface reveal and erase must be mutually exclusive modes, independent from the generic volume phase.");
