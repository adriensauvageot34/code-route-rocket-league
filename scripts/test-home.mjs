...
assert(/\.training-radar-fennec-surface-mask\s*\{[\s\S]*?position:\s*absolute;[\s\S]*?inset:\s*0;[\s\S]*?width:\s*100%;[\s\S]*?height:\s*100%;[\s\S]*?opacity:\s*0\.48;[\s\S]*?black 0 50%,[\s\S]*?transparent 53%[\s\S]*?mask-size:\s*200% 100%;/s.test(css), "The cumulative radar mask must remain full-canvas, unscaled and constantly visible.");
...
assert(/\.training-radar-fennec-surface\s*\{[\s\S]*?width:\s*100% !important;[\s\S]*?height:\s*100% !important;[\s\S]*?opacity:\s*1;[\s\S]*?brightness\(1\.18\) contrast\(1\.34\) saturate\(1\.2\);[\s\S]*?transition:\s*none;/s.test(css), "The calibrated surface image must fill only its measured frame without carrying the radar mask or opacity transition.");
