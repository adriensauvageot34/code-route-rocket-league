...
assert(/\.training-radar-ball-volume-surface\s*\{[\s\S]*?transparent 44%,[\s\S]*?black 49\.5% 50\.5%,[\s\S]*?transparent 53%/s.test(css), "The systematic ball surface scan must stay inside a narrow moving window.");
assert(/\.training-radar-ball-volume-contour\s*\{[\s\S]*?transparent 46%,[\s\S]*?black 49\.6% 50\.4%,[\s\S]*?transparent 52\.5%/s.test(css), "The systematic ball contour scan must reinforce the same narrow moving window.");
