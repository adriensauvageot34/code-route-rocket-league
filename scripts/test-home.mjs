...
assert(trainingScene.includes('className="training-radar-fennec-impact-frame"') && /\.training-radar-fennec-impact-frame\s*\{[\s\S]*?left:\s*40\.0669%;[\s\S]*?top:\s*34\.2818%;[\s\S]*?z-index:\s*2;[\s\S]*?width:\s*56\.0949%;[\s\S]*?height:\s*64\.9241%;/s.test(css), "The im-light must use its measured frame instead of the full scene canvas.");
