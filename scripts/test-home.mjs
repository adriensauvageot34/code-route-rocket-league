...
assert(/@keyframes training-ball-volume-surface-rtl\s*\{[\s\S]*?mask-position:\s*-30% 50%;[\s\S]*?mask-position:\s*130% 50%;/s.test(css), "The mirrored RTL ball mask must travel explicitly from right to left.");
assert(/\.training-radar-ball-volume-surface\s*\{[\s\S]*?transparent 40\.5%,[\s\S]*?black 47\.5% 52\.5%,[\s\S]*?transparent 58\.5%/s.test(css), "The systematic ball surface scan must expose a readable volumetric window without a full reveal.");
assert(/\.training-radar-ball-volume-contour\s*\{[\s\S]*?transparent 42\.5%,[\s\S]*?black 47\.8% 52\.2%,[\s\S]*?transparent 57%/s.test(css), "The systematic ball contour scan must reinforce the readable moving window without a full reveal.");
...
assert(css.includes("opacity: 0.68") && css.includes("opacity: 0.58") && css.includes("mask-position: 130% 50%") && css.includes("mask-position: -30% 50%"), "The aligned ball assets must expose a readable directional surface and contour pass.");
