...
assert(trainingScene.includes("training-radar-fennec-target") && trainingScene.includes("trainingFennecVolumeScanTarget.surfaceAsset") && trainingScene.includes("trainingFennecVolumeScanTarget.contourAsset") && trainingScene.includes("trainingFennecVolumeScanTarget.impactAsset") && trainingScene.includes("data-radar-target={trainingFennecVolumeScanTarget.id}") && !trainingScene.includes("data-volume-scan-phase") && !trainingScene.includes("data-tactical-active"), "The calibrated Fennec overlays must consume the global pass timeline without local React phases.");
...
assert(
  trainingParticleField.includes("getParticleScanDelayMs") &&
    ...
    trainingParticleField.includes("getTrainingRadarDelayForProgress(progress)"),
...
assert(!trainingParticleField.includes("TrainingRadarDirection") && !trainingParticleField.includes('direction === "ltr"'), "Particle timing must no longer retain an RTL branch.");
assert(trainingParticleField.includes("const directionalDriftX = particle.driftX * -1"), "Particles must always drift behind the LTR radar.");
...
assert(!trainingRadarSequence.includes("scheduleVolumePass") && !trainingRadarSequence.includes("scheduleTacticalPass") && !trainingRadarSequence.includes("setVolumeScan"), "The React sequence must not schedule any target or phase individually.");
for (const removedState of ["TrainingTacticalPhase", "TrainingVolumeScanPhase", "TrainingFennecSurfaceMode", "tacticalPhases", "volumeScanPhases", "fennecSurfaceMode", "fennecTacticalActive", "HIDDEN_TACTICAL_PHASES", "HIDDEN_VOLUME_SCAN_PHASES"]) {
  assert(!trainingRadarSequence.includes(removedState), `Legacy React radar phase state must be removed: ${removedState}.`);
}
...
assert(trainingGroundedActor.includes('"--training-volume-delay": `${target.scanDelayMs}ms`') && trainingGroundedActor.includes('"--training-tactical-delay": `${target.tacticalDelayMs}ms`') && trainingScene.includes('"--training-volume-delay": `${trainingFennecVolumeScanTarget.scanDelayMs}ms`') && trainingScene.includes('"--training-tactical-delay": `${trainingFennecVolumeScanTarget.tacticalDelayMs}ms`'), "Every target must expose its calibrated volume and tactical delays as CSS variables.");
...
assert(trainingGroundedActor.includes("TRAINING_VOLUME_SCAN_TIMING.ballActiveDurationMs") && trainingScene.includes("getTrainingRadarRangeTiming") && trainingScene.includes("rangeTiming.durationMs"), "The ball must keep its longer fixed reveal while the Fennec consumes the exact shared radar-range timing.");
...
const beginPassSequence = trainingRadarSequence.slice(trainingRadarSequence.indexOf("function beginPass"), trainingRadarSequence.indexOf("beginPass();"));
assert((trainingRadarSequence.match(/window\.setTimeout/g) ?? []).length === 1 && trainingRadarSequence.includes("passTimer = window.setTimeout(beginPass, nextPassDelayMs)") && trainingRadarSequence.includes("window.clearTimeout(passTimer)"), "The radar sequence must keep exactly one managed timer for the next complete pass.");
assert((beginPassSequence.match(/setSequence\(/g) ?? []).length === 1 && beginPassSequence.includes("passKey: current.passKey + 1") && beginPassSequence.includes("passMode,") && beginPassSequence.includes("running: true"), "passMode, passKey and running must change together in one React update.");
assert(/const nextPassDelayMs\s*=\s*TRAINING_RADAR_TIMING\.passDurationMs\s*\+\s*\(passMode === "tactical"[\s\S]*?TRAINING_RADAR_TIMING\.tacticalHoldDurationMs[\s\S]*?: 0\)/s.test(trainingRadarSequence), "Only the tactical pass may add the 1800ms post-radar hold.");
assert(!trainingGroundedActor.includes("tacticalPhase") && !trainingGroundedActor.includes("volumeScanPhase") && !trainingScene.includes("getTacticalPhase") && !trainingScene.includes("getVolumeScanPhase"), "Grounded actors and the scene must not receive obsolete React phase props.");
assert(
...
assert(css.includes('.training-scene[data-radar-active="true"][data-radar-pass-mode="volume"]') && css.includes('.training-scene[data-radar-active="true"][data-radar-pass-mode="tactical"]'), "Volume and tactical animations must derive from the same global pass state.");
for (const removedPhaseSelector of ['data-volume-scan-phase', 'data-tactical-phase', 'data-tactical-active', 'data-surface-scan-mode']) {
  assert(!css.includes(removedPhaseSelector) && !trainingGroundedActor.includes(removedPhaseSelector) && !trainingScene.includes(removedPhaseSelector), `Local React phase selector must be removed: ${removedPhaseSelector}.`);
}
assert(css.includes("training-object-tactical-wireframe-activate") && css.includes("opacity: 0.3") && css.includes("training-object-tactical-glow-activate") && css.includes("opacity: 0.09") && css.includes("forwards"), "Tactical wireframe and glow must persist until the global mode changes.");
...
assert(/@keyframes training-ball-tactical-contact\s*\{[\s\S]*?opacity:\s*0\.86;[\s\S]*?brightness\(1\.55\) saturate\(1\.32\)/s.test(css) && /@keyframes training-ball-tactical-active\s*\{[\s\S]*?opacity:\s*0\.52;[\s\S]*?brightness\(1\.3\) saturate\(1\.2\)/s.test(css), "The tactical ball must keep a strong contact flash and remain clearly gold while active.");
assert(css.includes("var(--training-volume-delay)") && css.includes("var(--training-tactical-delay)") && css.includes("var(--training-volume-hold-duration)") && css.includes("var(--training-volume-fade-duration)"), "Object animation delays, hold and fade must be composed entirely in CSS.");
for (const [fadeName, holdOpacity] of [["training-object-surface-fade", "0.22"], ["training-object-contour-fade", "0.14"], ["training-ball-volume-surface-fade", "0.32"], ["training-ball-volume-contour-fade", "0.2"], ["training-fennec-volume-surface-fade", "0.48"], ["training-fennec-volume-contour-fade", "0.14"]]) {
  assert(new RegExp(`@keyframes ${fadeName}\\s*\\{[\\s\\S]*?opacity:\\s*${holdOpacity};[\\s\\S]*?opacity:\\s*0;`).test(css), `Missing CSS hold-to-fade timeline: ${fadeName}.`);
}
...
assert(/@keyframes training-fennec-volume-surface-ltr\s*\{[\s\S]*?opacity:\s*0\.48;[\s\S]*?mask-position:\s*var\(--training-fennec-mask-start-position\) 50%;[\s\S]*?opacity:\s*0\.48;[\s\S]*?mask-position:\s*var\(--training-fennec-mask-end-position\) 50%;/s.test(css), "The temporary Fennec volume pass must preserve its 0.48 hold opacity while animating only the LTR mask geometry.");
assert(trainingRadarTargets.includes("getTrainingRadarRangeTiming") && trainingRadarTargets.includes("startProgress: 0.613") && trainingRadarTargets.includes("endProgress: 0.924") && trainingRadarTargets.includes("scanDelayMs: 1783") && trainingScene.includes("rangeTiming.durationMs"), "The Fennec volume lifecycle must use its calibrated start delay and exact linear duration across its measured width.");
assert(trainingScene.includes("--training-volume-scan-easing") && trainingScene.includes("--training-fennec-mask-start-position") && trainingScene.includes("--training-volume-delay") && trainingScene.includes("--training-tactical-delay"), "The Fennec scan must expose its exact range and pass delays to CSS.");
assert(fennecScanCss.includes('[data-radar-pass-mode="volume"]') && fennecScanCss.includes("training-fennec-volume-surface-ltr") && fennecScanCss.includes("training-fennec-volume-surface-fade") && fennecScanCss.includes("--training-fennec-detail-tail: 0.14"), "The Fennec volume scan must perform reveal, hold and fade on the global CSS timeline.");
assert(trainingScene.includes('className="training-fennec-base-frame"') && trainingScene.includes('className="training-fennec-base"') && !trainingScene.includes("fennecTacticalActive"), "The Fennec base and im-light must rely only on the global tactical mode.");
...
assert(/data-radar-pass-mode="tactical"[\s\S]*?\.training-fennec-base-frame\s*\{[\s\S]*?training-fennec-tactical-base-activate 650ms linear[\s\S]*?var\(--training-tactical-delay\) forwards;/s.test(css) && /data-radar-pass-mode="tactical"[\s\S]*?training-radar-fennec-impact[\s\S]*?training-fennec-tactical-impact-activate 650ms linear[\s\S]*?var\(--training-tactical-delay\) forwards;[\s\S]*?mask-image:\s*none;/s.test(css), "The global tactical pass must drive the synchronized Fennec base/im-light flicker at 2000ms.");
